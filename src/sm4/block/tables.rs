/*
* Expanded SM4 S-boxes
* Sbox table: 8bits input convert to 8 bits output
*/
pub const SBOX: [u8; 256] = [
    ,
];

#[allow(non_snake_case)]
const fn L(x: u32) -> u32 {
    x ^ x.rotate_left(2) ^ x.rotate_left(10) ^ x.rotate_left(18) ^ x.rotate_left(24)
}

// 1KB
/*
 * ltau_table[j] == L(sbox[j]) for j as u8
 */
pub const LTAU_TABLE8: [u32; 256] = {
    let mut table = [0; 256];
    let mut i: usize = 0;
    while i < 256 {
        table[i] = L(SBOX[i] as u32);
        i += 1;
    }
    table
};

// ltau_table16[j] == L(sbox[j]) for j as u16
// 65536 * 4 = 256 KB
pub const LTAU_TABLE16: [u32; 65536] = {
    let mut table = [0; 65536];
    let mut i: usize = 0;
    while i < 65536 {
        table[i] = L(SBOX[i & 0xff] as u32 | (SBOX[(i >> 8) & 0xff] as u32) * 256);
        i += 1;
    }
    table
};
